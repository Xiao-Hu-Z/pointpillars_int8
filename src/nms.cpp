...
inline int check_box2d(const Bndbox box, const float2 p) {
    ...
    return (fabs(rot_x) < box.w / 2 + MARGIN &&
            fabs(rot_y) < box.l / 2 + MARGIN);
}
...
inline void rotate_around_center(const float2 &center, const float angle_cos,
...
inline float box_overlap(const Bndbox &box_a, const Bndbox &box_b) {
    ...
    float a_dx_half = box_a.w / 2, b_dx_half = box_b.w / 2,
          a_dy_half = box_a.l / 2, b_dy_half = box_b.l / 2;
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            flag = intersection(box_a_corners[i + 1], box_a_corners[i],
                                ...
                                cross_points[cnt]);
            if (flag) {
                poly_center = {poly_center.x + cross_points[cnt].x,
    ...
    for (int k = 0; k < 4; k++) {
        if (check_box2d(box_a, box_b_corners[k])) {
            poly_center = {poly_center.x + box_b_corners[k].x,
                           poly_center.y + box_b_corners[k].y};
            cross_points[cnt] = box_b_corners[k];
            cnt++;
        }
        if (check_box2d(box_b, box_a_corners[k])) {
            poly_center = {poly_center.x + box_a_corners[k].x,
    ...
    for (int j = 0; j < cnt - 1; j++) {
        for (int i = 0; i < cnt - j - 1; i++) {
            if (atan2(cross_points[i].y - poly_center.y,
                      cross_points[i].x - poly_center.x) >
                atan2(cross_points[i + 1].y - poly_center.y,
                      cross_points[i + 1].x - poly_center.x)) {
                temp = cross_points[i];
...
int nms_cpu(std::vector<Bndbox> bndboxes, const float nms_thresh,
    ...
    std::sort(bndboxes.begin(), bndboxes.end(),
              [](Bndbox boxes1, Bndbox boxes2) {
                  return boxes1.score > boxes2.score;
              });
