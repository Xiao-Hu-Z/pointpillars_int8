...
class Object3d(object):
    ...
    def __init__(self, label_file_line, is_pred = False):
        data = label_file_line.split(" ")
        ...
        self.conf = 1.0
        if is_pred:
            self.conf = data[15]    # prediction confidence [0..1] 
...
def read_pred(pred_filename, conf_limit = [0.7, 0.5, 0.5]):
    lines = [line.rstrip() for line in open(pred_filename)]
    objects = list()
    for line in lines:
        pred = Object3d(line, True)
        if pred.conf > conf_limit[0] and pred.type == 'Car':
            objects.append(pred)
        if pred.conf > conf_limit[1] and pred.type == 'Pedestrian':
            objects.append(pred)
        if pred.conf > conf_limit[2] and pred.type == 'Cyclist':
            objects.append(pred)
    return objects
